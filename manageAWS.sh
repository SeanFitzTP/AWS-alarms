...
function cmd_download_secrets() {
  ...
  if [[ ! -f .secrets.json ]]; then
    ...
    aws secretsmanager get-secret-value --secret-id ci-example | jq '.SecretString | fromjson' >.secrets.json
  fi
}
...
#    echo "Executing on Mac. Native image will build in a docker container."
  MVN_FLAGS="-Dquarkus.native.container-build=true -Dquarkus.native.builder-image=quay.io/quarkus/ubi-quarkus-native-image:21.0.0-java11"
#  else
...
#  mvn -Dtest=OutsideInCloudLoanProviderTest test
}
...
function generate_dot_env() {
  ...
  export GET_FUNCTION_NAME="$(get_physical_resource_id_for_resource_type "$(get_stack_name)" ExampleFn AWS::Lambda::Function)"
